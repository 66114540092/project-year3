...
- Tournament เก็บข้อมูลทัวร์นาเมนต์ ประกอบด้วย id, name, bracket_size, status, created_by (FK), pin_code (6 หลัก), voting_duration_seconds (ระยะเวลาโหวต), current_round, total_rounds มีความสัมพันธ์แบบ many-to-one กับ User
- TournamentParticipant เก็บข้อมูลผู้เข้าร่วมห้อง Lobby ประกอบด้วย id, tournament_id (FK), user_id (FK, nullable), nickname, session_key, joined_at มีความสัมพันธ์แบบ many-to-one กับ Tournament
- Competitor เก็บข้อมูลผู้เข้าแข่งขัน ประกอบด้วย id, tournament_id (FK), name, image, seed มีความสัมพันธ์แบบ many-to-one กับ Tournament
- Match เก็บข้อมูลแมตช์ ประกอบด้วย id, tournament_id (FK), round_number, index_in_round, competitor1 (FK), competitor2 (FK), winner (FK), is_finished, started_at (เวลาเริ่มแมตช์)
- MatchVote เก็บข้อมูลการโหวต ประกอบด้วย id, match_id (FK), user_id (FK), choice ('1' หรือ '2'), created_at มี unique constraint (match, user) ป้องกันโหวตซ้ำ
...
3.6.1 กระบวนการโหวต Real-time มีลำดับดังนี้
1) ผู้ใช้คลิกโหวตที่รูปผู้เข้าแข่งขัน
2) JavaScript ส่ง AJAX POST request ไปยัง /vote/
3) Django ตรวจสอบว่าผู้ใช้โหวตแล้วหรือยัง (ใช้ update_or_create)
4) ถ้ายังไม่เคยโหวต ระบบบันทึก MatchVote
5) Server ส่ง JSON response กลับมา พร้อม percentage และจำนวน votes
6) JavaScript อัปเดตหน้าจอ
7) ทุกๆ 2 วินาที browser จะ poll ข้อมูลใหม่จาก server
8) เมื่อหมดเวลา Server จะ auto-finish และส่ง redirect_url

(รูปที่ 3.5 Sequence Diagram กระบวนการโหวต)


3.6.2 กระบวนการ Join via PIN มีลำดับดังนี้
1) ผู้ใช้ไปที่หน้า /tournaments/join/
2) กรอก PIN Code 6 หลัก
3) Server ตรวจสอบว่า PIN ถูกต้องและทัวร์นาเมนต์อยู่ในสถานะ waiting
4) ถ้าถูกต้อง จะ redirect ไปหน้ากรอก Nickname
5) ผู้ใช้กรอก Nickname
6) Server สร้าง TournamentParticipant record
7) Redirect ไป Waiting Lobby

(รูปที่ 3.6 Sequence Diagram กระบวนการ Join via PIN)
